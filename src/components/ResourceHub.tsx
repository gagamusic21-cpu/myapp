...
import { EntityRef, coursesFor, docsFor, examsFor, photosFor, downloadExam, downloadDoc } from "../data/resources";
import { Course, ExamPaper, ResourceDoc } from "../data/types";
import { useStore } from "../lib/store";
import { EmptyState, KindBadge, LoadMore, Modal, Tabs } from "./ui";
import ChatRoom from "./ChatRoom";
import Lightbox from "./Lightbox";
import { IBook, ICal, IClock, IDoc, IDownload, IExam, IEye, IKey, ICheck, IX } from "./icons";
...
export default function ResourceHub({ entity, roomLabel }: { entity: EntityRef; roomLabel: string }) {
  ...
  const courses = useMemo(() => coursesFor(entity), [entity]);
  const exams = useMemo(() => examsFor(entity), [entity]);
  const docs = useMemo(() => docsFor(entity), [entity]);
  const photos = useMemo(() => photosFor(entity), [entity]);

  const [visible, setVisible] = useState(PAGE);
  const [examFilter, setExamFilter] = useState("All");
  const [yearFilter, setYearFilter] = useState("All");
  const [docQuery, setDocQuery] = useState("");
  const [openExam, setOpenExam] = useState<ExamPaper | null>(null);
  const [examAnswers, setExamAnswers] = useState(false);
  const [openDoc, setOpenDoc] = useState<ResourceDoc | null>(null);
  ...
  const tabDocs = useMemo(() => {
    const kindMap: Record<string, ResourceDoc["kind"][]> = {
    ...
    <div>
      ...
      <div className="pt-6">
        ...
        {(tab === "exams" || tab === "answers") && (
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-5">
              ...
              {tab === "answers" && (
                <p className="text-[0.74rem] font-mono text-pine-600 dark:text-gold-400 flex items-center gap-1"><IKey size={13} /> papers with verified answer keys</p>
              )}
            </div>
        ...
        {tab === "courses" && (
          <div className="grid sm:grid-cols-2 gap-3.5">
            {courses.map((c) => (
              <CourseCard key={c.id} c={c} onExams={() => { changeTab("exams"); }} />
            ))}
          </div>
        )}
        ...
        {isDocTab && (
          <div>
            ...
            {tabDocs.length === 0 ? <EmptyState text="Uploads are added regularly by moderators — check back soon." /> : (
                ...
                {tabDocs.slice(0, visible).map((doc) => (
                  <div key={doc.id} className="card-surface rounded-xl px-4 py-3 flex items-center gap-3.5 hover:shadow-md hover:-translate-y-px transition-all">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="font-mono text-[0.68rem] text-inksoft dark:text-pine-200/55 mt-0.5">{doc.course} · {doc.pages} pages · {doc.size} · updated {doc.updated}</p>
                    </div>
...
function CourseCard({ c, onExams }: { c: Course; onExams: () => void }) {
  return (
    <article className="card-surface rounded-xl p-4 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
      ...
      <button onClick={onExams} className="btn-ghost !py-1.5 !px-3 text-[0.76rem] mt-3.5"><IBook size={13} /> Browse past papers for this course</button>
    </article>
  );
}

function ExamPaperView({ paper, showAnswers, onToggle }: { paper: ExamPaper; showAnswers: boolean; onToggle: (v: boolean) => void }) {
  const { toast } = useStore();
  return (
    <div>
      {/* paper header — styled like a real examination paper */}
      <div className="border-2 border-pine-700/60 dark:border-pine-400/40 rounded-lg overflow-hidden mb-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-pine-700/25 dark:divide-pine-400/20 text-center font-mono text-[0.7rem]">
          {[["Course", `${paper.courseCode}`], ["Semester", paper.semester], ["Time", paper.duration], ["Marks", `${paper.marks}`]].map(([k, v]) => (
            <div key={k} className="px-2 py-2"><p className="uppercase tracking-widest text-[0.58rem] text-inksoft dark:text-pine-200/50">{k}</p><p className="font-bold mt-0.5">{v}</p></div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        {!paper.hasAnswers && (
          <p className="text-[0.7rem] font-mono text-clay-500">Community-verified answer key not yet available for this paper.</p>
        )}
      </div>
        ...
        {paper.questions.map((q, i) => (
          <li key={i} className="card-surface rounded-xl p-4">
            <p className="font-semibold text-[0.92rem] leading-relaxed">
              <span className="font-mono text-pine-600 dark:text-gold-400 mr-2">Q{i + 1}.</span>{q.q}
            </p>
            <div className="grid sm:grid-cols-2 gap-2 mt-3">
              {q.o.map((o, j) => {
                const isAns = showAnswers && j === q.a;
                return (
            ...
            {showAnswers && q.e && (
              <p className="mt-2.5 text-[0.78rem] text-inksoft dark:text-pine-200/65 border-l-2 border-gold-500 pl-3 italic">{q.e}</p>
            )}
          </li>
        ))}
      </ol>
...
function DocPreview({ doc, onDownload }: { doc: ResourceDoc; onDownload: () => void }) {
  const { toast } = useStore();
  return (
    <div>
      <div className="rounded-xl border-2 border-dashed border-pine-600/30 dark:border-pine-400/25 p-5 text-center mb-4">
        ...
        <p className="font-mono text-[0.68rem] text-inksoft dark:text-pine-200/50">Last updated {doc.updated}</p>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={() => toast("Opening full viewer (demo) — download for the complete file", "info")} className="btn-primary !py-2"><IEye size={15} /> Open</button>
